        ...
        <div class="content bg-white border">
            <div class="m-5">
                @foreach ($dtMenu as $item)
                    <form action="{{ route('menu.update', $item->Menu_id) }}" method="POST" class="mb-3">
                        @csrf
                        <div class="mb-3">
                            <label for="name" class="form-label">Nama</label>
                            <input type="text" value="{{ $item->Menu_name }}" name="name" id="name"
                                class="form-control">
                        </div>
                            ...
                            <div class="col-md-8 {{ $errors->has('category') ? 'has-error' : '' }}">
                                <select name="category" id="category" class="form-control"
                                    ...
                                    <option value="Master Menu"
                                        {{ $item->Menu_category == 'Master Menu' ? 'selected' : '' }}>Master Menu
                                    </option>
                                    <option value="Sub Menu" {{ $item->Menu_category == 'Sub Menu' ? 'selected' : '' }}>
                                        Sub Menu</option>
                                </select>
                            </div>
                        </div>
...
@section('style')
    <style>
        @if ($item->Menu_category == 'Sub Menu')
            #hidden_div {
                display: block;
            }
        @else
